...
BioCC_input <- read.table(file = "/Users/herber4/Desktop/SF3B1/frac_counts.txt", sep = "\t", header = T)
...
#set groups
Groups <- as.factor(c(rep("K7_Cyto", 2), rep("K7_Nuc", 2), rep("K7_Whole", 2),
                      rep("WT_Cyto", 2), rep("WT_Nuc", 2), rep("WT_Whole", 2)))
...
fracs_cpms <- cpm(rpk.norm.g, log = F)
...
MTvsWT <- makeContrasts((GroupsK7_Cyto)-(GroupsWT_Cyto),levels = design.mat)
...
write.table(ddx_P1, file = "/Users/herber4/Desktop/SF3B1/k562_fracs/k7_cyto_vs_cyto.txt", sep = "\t")
cyto <- ddx_P1[["table"]]

ggplot(filter(cyto, logCPM > 2 & PValue < .05), aes(x = logFC, y = -log(PValue, 10))) +
  geom_point() +
  theme_classic()
...
tsorted <- sort(tvar, decreasing = TRUE, index.return = TRUE)$ix[1:500]
...
pheatmap(log, cluster_rows = T, cluster_cols = T,
         ...
         fontsize_col = 3,
         color = colorRampPalette(c("red", "black", "green"))(500))
